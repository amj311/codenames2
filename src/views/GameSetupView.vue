...
export default {
    ...
    methods: {
        async doGameAction(action, data) {
            const { data: resData } = await api.post('/room/' + this.gameStore().gameRoomId + '/game-action/' + action, {
                userId: this.user.id,
                data,
            });
            if (!resData.success) {
            ...
            this.gameStore().setGameState(resData.game);
            this.gameStore().setRoomState(resData.room);
        ...
        async claimCaptain(teamCode) {
            try {
                await this.doGameAction('makeUserCaptain', {
                    userId: this.user.id,
                    teamCode,
                });
            }
          ...
          <div id="teamLists">
            ...
            >
              ...
              >
                ...
                <div>{{ teamData.captainId.nickname }}</div>
              </div>
            </div>
          </div>
        </div>
                ...
                >
                    ...
                    >
                        ...
                        <div v-if="gameState.teams[teamCode].captainId">
                            {{ gameStore().getUserById(gameState.teams[teamCode].captainId).nickname }}
                        </div>
                        <div v-else>
        ...
        >
            ...
            >
                ...
                <div style="text-align:center">
                    ...
                    <p>Scan the code, or visit <b>{{ hostUrl.host }}</b> and enter the room code.</p>
...
.team-caption-option.no-captain .ninja {
    opacity: .5;
}
