...
class GameplayHandler {
  ...
  advanceTurn(game) { this.vue.advanceTurn(game) }
...
export default {
  ...
  mounted() {
    this.printSecretKey();
    if (!this.gameState.state.isGameOver) this.advanceTurn(this.gameState);
  },
  ...
  methods: {
    ...
    setUserAsCaptain(teamCode) {
      if (this.user.isHost || this.userCaptainOfTeam) return;
      console.log("setting team captain")
      // this.gameStore().doGameAction('  method', {Captan",
      //   args: [true, teamCode, this.user]
      // })
    }
  }
}
...
<template>
  <div id="boardWrapper">
    <div
      ...
      class="ui-raised"
    >
      <div id="roomCode"><i class="material-icons">tap_and_play</i><span>{{ gameStore().gameRoomId }}</span></div>
