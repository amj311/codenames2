...
<template>
	...
	>
		<div
			class="ui-block"
			style="width: min(calc(100vw - 4rem), 50rem)"
		>
			<div style="display: flex; align-items: center; justify-content: space-between;">
				<h3>Turns Play-by-play</h3>
				<button
					class="text"
			...
			<div style="max-height: 60vh;  overflow: auto; padding-right: 1em;">
				<div
					...
					:key="turn.turnCount"
					style="margin-top: 1em; display: flex; flex-direction: column; gap: .5em;"
				>
					...
					<div>
						Intended:
						<span class="text-code">{{ turn.matchingCardIds.map(id => gameState.cards.find(c => c.id ===
							id)?.word).join(', ') }}</span>
					</div>
