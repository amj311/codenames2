...
export default class GameRoom {
  ...
  get clientInterface() {
    return {
      async ping(userId: string, newUserData: any) { },
  ...
  async doRoomAction(userId, action: keyof typeof this.clientInterface, data: any) {
    const user = this.users.get(userId);
    if (!user) return;
    await this.clientInterface[action](user, data);
  }
  ...
  getPlayers() {
    const players: any[] = [];
    Array.from(this.users.values()).forEach((c) => {
